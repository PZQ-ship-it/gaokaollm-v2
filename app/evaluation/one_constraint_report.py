"""Summarize one-constraint diagnostic benchmark results."""
...
DEFAULT_PERSONAS = Path(
    "gaokaollm_bench/sample_data/iceberg_personas_1constrain_real_db_30.json"
)
DEFAULT_OUTPUT_ROOT = Path("gaokaollm_bench/outputs/method_score_arena_1constrain")
DEFAULT_CSV = Path("app/evaluation/results/one_constraint_summary.csv")
DEFAULT_MD = Path("app/evaluation/results/one_constraint_summary.md")
...
def _load_persona_meta(path: Path) -> dict[str, dict[str, Any]]:
    ...
    for item in data:
        ...
        meta[str(item["case_id"])] = {
            "diagnostic_axis": str(background.get("diagnostic_axis") or ""),
            "preference_axis": str(background.get("preference_axis") or ""),
            "score_band": str(background.get("score_band") or ""),
            "source_score_band": str(background.get("source_score_band") or ""),
        }
    return meta
        ...
        for row in summary.get("rows") or []:
            ...
            grouped[
                (
                    ...
                    case_meta["diagnostic_axis"],
                    case_meta["score_band"],
                )
            ].append({**row, **case_meta})

    rows = []
    for (model, target, axis, score_band), items in sorted(grouped.items()):
...
def write_markdown(rows: list[dict[str, Any]], path: Path) -> None:
    lines = [
        "# One-Constraint Diagnostic Summary",
        "",
        "Grouped by model, target, diagnostic axis and requested score band.",
        "",
        "| Model | Target | Axis | Band | Cases | Completed | Failed | Success | Gain | Hallucination | Turns |",
        "| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in rows:
        lines.append(
            f"| {row['model']} | {row['target']} | {row['diagnostic_axis']} | {row['score_band']} | "
