...
DEFAULT_OUTPUT_ROOT = Path("gaokaollm_bench/outputs/major_probe_architecture_trials")
DEFAULT_TRAIN_PATH = Path("gaokaollm_bench/outputs/major_probe_classification_ablation/data/raw_train_only.jsonl")
DEFAULT_VAL_PATH = Path("gaokaollm_bench/outputs/major_training/splits/val.jsonl")
DEFAULT_EMBEDDINGS_PATH = Path("gaokaollm_bench/outputs/major_training/embeddings_union_val_filled.npz")
PROMOTION_MACRO_F1 = 0.6862
PROMOTION_ACCURACY = 0.7088
...
class ArchitectureConfig:
    ...
    dropout: float
    activation: str = "gelu"
    class_weight: str = "sqrt_balanced"
...
def _validate_inputs(train_path: Path, val_path: Path, embeddings_path: Path) -> dict[str, Any]:
...
def summarize_trials(output_root: Path) -> dict[str, Any]:
        ...
        runs.append(
            {
                ...
                "architecture_name": ablation_config.get("architecture_name") or path.parent.name.rsplit("_s", 1)[0],
    ...
    for architecture_name, group_runs in grouped.items():
        macro_values = [float(row["best_val_macro_f1"]) for row in group_runs if row.get("best_val_macro_f1") is not None]
        accuracy_values = [float(row["best_val_accuracy"]) for row in group_runs if row.get("best_val_accuracy") is not None]
        top3_values = [
            float(row["best_val_top3_accuracy"]) for row in group_runs if row.get("best_val_top3_accuracy") is not None
        ]
        epoch_values = [float(row["best_epoch"]) for row in group_runs if row.get("best_epoch") is not None]
    ...
    aggregates.sort(
        key=lambda row: (
            float("-inf") if row["best_val_macro_f1_mean"] is None else float(row["best_val_macro_f1_mean"]),
            float("-inf") if row["best_val_accuracy_mean"] is None else float(row["best_val_accuracy_mean"]),
        ),
        reverse=True,
    )
    return {
...
def write_summary(output_root: Path, payload: dict[str, Any]) -> None:
    ...
    candidates = [row for row in payload["aggregates"] if row.get("promotion_candidate")]
...
def main() -> None:
    ...
    print("Saved architecture summary to", output_root / "architecture_trials_summary.json")
