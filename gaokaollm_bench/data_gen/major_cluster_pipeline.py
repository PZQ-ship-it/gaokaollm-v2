...
def _iter1_baseline(tree: dict[str, Any], unassigned: list[str], *, filter_observed: bool) -> dict[str, Any]:
...
def _iter3_consistency(tree: dict[str, Any], suggestions: list[dict[str, Any]], *, high_sim: float) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "high_similarity_mismatch": bool(similarity >= high_sim and rule_status == "mismatch"),
            }
        )
    return rows
...
async def _iter4_threshold_sweep(
    ...
    *,
    ...
    for attach_threshold, new_sibling_threshold in sweep:
        ...
        counts = {"attach_to_leaf": 0, "suggest_new_sibling_leaf": 0, "manual_review": 0}
...
async def main() -> None:
    ...
    sample = baseline["unassigned"][: args.sample_size] if args.sample_size else baseline["unassigned"]
