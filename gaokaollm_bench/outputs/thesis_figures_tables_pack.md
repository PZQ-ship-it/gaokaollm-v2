...
| 贡献层 | 核心对象 | 论文表达重点 |
| --- | --- | --- |
| 数据贡献 | PostgreSQL 招生快照、专业层级本体、学校-专业质量/实力证据层、专业就业结果画像、经人工审校的地域层级画像 | 把分数、位次、学费、学校-专业质量/实力、就业结果和地域节点转成可核验事实证据 |
...
| 学校-专业证据 | 学校-专业质量/实力证据放宽 | `major_strength_rank`、评级、质量得分、排名、评估、特色、满意度 | `strength_relax` 是窄口径排名型探针，`major_quality_relax` 是综合质量画像探针 |
...
| `school_strength_v1` | 扩展实验 | 学校-专业证据轴：窄口径排名型实力放宽 | `1.000 / 15.000 / 0.000 / 5.00` | `0.000 / 0.000 / 0.000 / 11.00` | 验证排名/评级证据可接入同一谈判框架 |
| `tuition_value_v1` | 扩展实验 | 预算性价比放宽 | `1.000 / 1.000 / 0.000 / 5.00` | `0.000 / 0.000 / 0.000 / 11.00` | 验证预算边界可通过成本收益证据校准 |
| `major_quality_v1` | 扩展实验 | 学校-专业证据轴：综合质量画像放宽 | `1.000 / 16.000 / 0.000 / 5.00` | `0.000 / 0.000 / 0.050 / 11.00` | 验证综合质量画像支持细粒度妥协 |
...
说明：`school_strength_v1` 与 `major_quality_v1` 不应解释为两个完全正交的偏好轴。二者同属学校-专业证据扩展，前者是窄口径排名型对照，后者是融合专业排名、学科评估、特色/重点和满意度的综合画像主叙述。
...
| 扩展实验定位 | 五组扩展实验支撑数据贡献和框架可扩展性，不替代主实验；其中学校-专业证据扩展包含窄口径排名探针与综合质量画像探针。 |
