...
???? Agent ?????? `前置语义归一层 -> 约束解析器 -> LLM 引导的机会规划器 -> 确定性证据探针 -> 证据谈判器`????????????? v1 ???????LLM ???????????????????????????????????????????????????????????LLM 不生成学校、专业、分数、位次等事实候选，只输出 `probe_plan`、`opportunity_rankings` 和 `clarification_hint`；Agent 不读取 `implicit_flexibilities`、`volunteer_set` 或 `axis_flexibilities`。
...
| --- | --- | --- | --- |
| ??????? | `semantic_normalizer` | ?????? | ???????????????? |
| ????? | `gatekeeper` | ?????????? | ?????? hard-constraint baseline |
| LLM ???????? | `radar` planner | ?????baseline ???? | `probe_plan`?`opportunity_rankings`?`clarification_hint` |
| ??????? | SQL probes | ???????????? | ?????? Pareto opportunities |
| ????? | `negotiator` | baseline?opportunities?????????? | ????????????????? |

LLM ??????????????????????? probe_plan?opportunity_rankings ? clarification_hint?Agent ??? implicit_flexibilities?volunteer_set ? axis_flexibilities? `region_tree_relax`?`major_quality_relax`?`employment_outcome_relax` ????????????LLM ?????????????????????

Benchmark ?? simulator ? evaluator ??? agent-like role??????????? Agent ???????????????simulator ???????evaluator ???? transcript??? Agent ??????????????????

### 3.1 Hidden Persona ?? 边界

Agent 的输入只来自用户显式话语和 PostgreSQL 查询结果。Agent 不读取 `implicit_flexibilities` 或 `volunteer_set`。这些 hidden fields 只作为 Benchmark 的 evaluator ground truth，用来判断 Agent 是否通过对话触发了隐藏妥协。

该边界非常重要：如果 Agent 直接读取 hidden persona，实验就会变成信息泄漏，而不是偏好妥协能力评测。
