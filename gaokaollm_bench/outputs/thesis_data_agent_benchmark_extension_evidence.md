...
本文档服务毕业论文“数据 + Agent + Benchmark”贡献结构，整理三组扩展实验的逐例证据链：`school_strength_v1`、`tuition_value_v1`、`major_quality_v1`。

当前论文主实验仍是 `major_geo_v1 + risk_band_v1`。本附录中的三组实验不替代主实验，而是证明同一套数据生成、Agent 探测、Benchmark 沙盒和事实/过程评价流程，可以接入更多真实 PostgreSQL 证据维度。
...
三组实验均使用 `app_pareto` 与 `hard_constraint` 对照。`app_pareto` 表示接入证据驱动 Pareto 谈判的业务 Agent；`hard_constraint` 表示只报告当前显性硬约束下可达志愿、不主动提出放宽谈判的 baseline。
...
论文写作时建议把这三组称为扩展实验：它们证明“数据证据维度可扩展”，而不是把主实验从 `major_geo_v1 + risk_band_v1` 改写为五组并列主实验。
...
三组扩展实验中，`hard_constraint` 都没有成功触发隐藏妥协：
...
扩展实验支持“数据 + Agent + Benchmark”的论文贡献结构：数据层提供可核验的分数、位次、学费和专业质量证据；Agent 层通过 `radar` 探测 `strength_relax`、`tuition_value_relax`、`major_quality_relax` 等 Pareto 机会；Benchmark 层用冰山画像、多轮沙盒和事实/过程联合评价检验这些机会是否真的触发用户隐藏妥协。

因此，`school_strength_v1`、`tuition_value_v1` 和 `major_quality_v1` 适合作为扩展实验写入论文，证明当前框架不只适用于专业/地域和风险组合，也可以接入新的数据证据维度。主实验结论仍应以 `major_geo_v1 + risk_band_v1` 为中心，扩展实验用于说明框架的可扩展性和数据贡献价值。
