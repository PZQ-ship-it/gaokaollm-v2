...
from gaokaollm_bench.constrains.enums import ProbeClassWeight, ProbeModelKind, values
...
DATASETS = {
    "raw": DatasetConfig(
        name="raw",
        train_path=MAJOR_RAW_TRAIN_ONLY_JSONL,
        embeddings_path=MAJOR_EMBEDDINGS_UNION_VAL_FILLED,
        method_family="training_data",
    ),
}
DEFAULT_DATASETS = ["raw"]
DEFAULT_MODEL_KINDS = [ProbeModelKind.LINEAR.value, ProbeModelKind.MLP.value]
DEFAULT_CLASS_WEIGHTS = [
    ProbeClassWeight.NONE.value,
    ProbeClassWeight.SQRT_BALANCED.value,
]
...
class RunConfig:
    ...
    def name(self) -> str:
        model_part = (
            "linear" if self.model_kind == "linear" else f"mlp_h{self.hidden_dim}"
        )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run probe-only major classification ablations"
    )
    parser.add_argument(
        "--output-root", default=str(MAJOR_PROBE_CLASSIFICATION_ABLATION_DIR)
    )
    ...
    parser.add_argument(
        "--model-kind",
        action="append",
        choices=[ProbeModelKind.LINEAR.value, ProbeModelKind.MLP.value],
        default=[],
    )
    parser.add_argument(
        "--class-weight",
        action="append",
        choices=values(ProbeClassWeight),
        default=[],
...
def validate_run_inputs(run: RunConfig, val_path: Path) -> dict[str, Any]:
    ...
    assert_complete_embedding_coverage(
        train_rows + val_rows, run.dataset.embeddings_path
    )
    return {
