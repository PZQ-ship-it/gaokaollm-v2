...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze major probe validation errors"
    )
    parser.add_argument(
        "--input-jsonl",
        default="gaokaollm_bench/outputs/major_training/splits/val.jsonl",
    )
    parser.add_argument(
        "--embeddings", default="gaokaollm_bench/outputs/major_training/embeddings.npz"
    )
    parser.add_argument(
        "--probe", default="gaokaollm_bench/outputs/major_training_probe/best_probe.pt"
    )
    parser.add_argument(
        "--label-map",
    ...
    parser.add_argument(
        "--major-tree", default="gaokaollm_bench/outputs/major_tree_final_reviewed.json"
    )
    parser.add_argument(
        "--output-dir", default="gaokaollm_bench/outputs/major_probe_error_analysis"
    )
...
def analyze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report = _classification_report(
        y_true_array, y_pred_array, labels=labels, target_names=target_names
    )
    ...
    summary = {
        ...
        "topk_miss_count": sum(
            1 for item in per_sample if not item["topk_contains_gold"]
        ),
        "ambiguous_count": sum(
            1 for item in per_sample if item["ambiguous_compound"]["is_ambiguous"]
        ),
...
def main() -> None:
    ...
    _write_json(
        output_dir / "classification_report.json", result["classification_report"]
    )
