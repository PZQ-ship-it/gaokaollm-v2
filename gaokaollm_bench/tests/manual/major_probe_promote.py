...
from gaokaollm_bench.constrains.metrics import (
    DEFAULT_PROMOTION_ACCURACY,
    DEFAULT_PROMOTION_MACRO_F1,
)
from gaokaollm_bench.constrains.paths import MAJOR_TRAINING_PROBE_DIR
from gaokaollm_bench.constrains.probe import DEFAULT_PROBE_ARTIFACT_FILES

DEFAULT_FILES = DEFAULT_PROBE_ARTIFACT_FILES


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Promote a major probe experiment to default"
    )
    parser.add_argument("--candidate-dir", required=True)
    parser.add_argument("--default-dir", default=str(MAJOR_TRAINING_PROBE_DIR))
    parser.add_argument("--archive-dir", default=None)
    parser.add_argument(
        "--min-macro-f1", type=float, default=DEFAULT_PROMOTION_MACRO_F1
    )
    parser.add_argument(
        "--min-accuracy", type=float, default=DEFAULT_PROMOTION_ACCURACY
    )
...
def _metric_gate(
    candidate_dir: Path, *, min_macro_f1: float, min_accuracy: float
) -> dict:
...
def main() -> None:
    ...
    gate = _metric_gate(
        candidate_dir, min_macro_f1=args.min_macro_f1, min_accuracy=args.min_accuracy
    )
    if args.dry_run:
        print(
            json.dumps(
